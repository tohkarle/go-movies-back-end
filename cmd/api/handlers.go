...
func (app *application) InsertMovie(w http.ResponseWriter, r *http.Request) {
	...
	err := app.readJSON(w, r, &movie) // JSON response data is read into movie.
	if err != nil {
		app.errorJSON(w, err)
		return
	}
	...
	// Try to get an image.
	movie = app.getPoster(movie) // 'movie' is now populated with image.
...
func (app *application) getPoster(movie models.Movie) models.Movie {
	...
	client := &http.Client{} // This is from the standard library in Go.
	...
	json.Unmarshal(bodyBytes, &responseObject) // Unmarshal bodyBytes into responseObject. Unmarshal refers to the process of converting a data format (such as JSON) back into a data structure (such as a struct or slice).
...
func (app *application) UpdateMovie(w http.ResponseWriter, r *http.Request) {
	...
	err := app.readJSON(w, r, &payload) // Read the values into payload.
	if err != nil {
		app.errorJSON(w, err)
		return
	}
	...
	app.writeJSON(w, http.StatusAccepted, resp) // Passing a response back to front end.
}

func (app *application) DeleteMovie(w http.ResponseWriter, r *http.Request) {
	id, err := strconv.Atoi(chi.URLParam(r, "id")) // Get the 'id' from request url parameter.
	if err != nil {
		app.errorJSON(w, err)
		return
	}
...
func (app *application) moviesGraphQL(w http.ResponseWriter, r *http.Request) {
	...
	// Get the query from the request (not in the form of JSON).
